...
from model.point3d import Point3D
from model.line_segment3d import LineSegment3D
...
class Facet3D(object):
    ...
    def slice_at_z(self, z, quanta=1e-3):
        ...
        u1 = (z-p1[2])/(p2[2]-p1[2])
        u2 = (z-p3[2])/(p4[2]-p3[2])
        px =  round(p1[0]+u1*(p2[0]-p1[0]),5)
        py =  round(p1[1]+u1*(p2[1]-p1[1]),5)
        qx =  round(p3[0]+u2*(p4[0]-p3[0]),5)
        qy =  round(p3[1]+u2*(p4[1]-p3[1]),5)
        line = ((px, py), (qx, qy))
        pt = (round(px + norm2d[0],5), round(py + norm2d[1],5))
        line = self._clockwise_line(line, pt)
        return line
